...
export default function BoookmarkButton({post_id}) {
  ...
  return (
      ...
      <button
        type="button"
        className="relative inline-flex items-center rounded  bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 focus:z-10  focus:ring-indigo-500"
      >
        <BookmarkIcon onClick={isBookmarked?removeBookmark:addBookmark} className= {`-ml-1 mr-2 h-5 w-5 ${isBookmarked?"text-green-400":"text-gray-400"}`} aria-hidden="true" />
      </button>
